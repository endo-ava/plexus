...
| コマンド | 説明 |
|---|---|
| `sudo ./install.sh install` | デフォルトパスでインストール・起動 |
| `sudo ./install.sh install /opt/plexus` | パス指定でインストール |
| `sudo ./install.sh uninstall` | 停止・登録解除 |
| `./install.sh status` | 状態確認（root 不要） |
